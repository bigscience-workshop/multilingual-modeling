...
# load tokenizer
tokenizer = BertTokenizer.from_pretrained(args.tokenizer, cache_dir=args.cache_dir)
...
def tokenize_function(examples):
    if KLUE_TASKS[args.klue_task].klue_split == "ynat":
        ...
        return tokenizer(f'{examples["sentence1"]} {tokenizer.sep_token} {examples["sentence2"]}', max_length=128, padding="max_length", truncation=True)
    elif KLUE_TASKS[args.klue_task].klue_split == "nli":
        return tokenizer(f'{examples["premise"]} {tokenizer.sep_token} {examples["hypothesis"]}', max_length=128, padding="max_length", truncation=True)
...
def load_model(args):
    if KLUE_TASKS[args.klue_task].model_type == "seq-cls":
        return BertForSequenceClassification.from_pretrained(args.pretrained_model, 
                                                             num_labels=KLUE_TASKS[args.klue_task].num_labels,
                                                             pad_token_id=0,
