...
parser.add_argument("--en_adpt_dir", type=str)
...
def load_config_adapters(model, config, lang_adapter_dir):
    pretrained_adapter_name = model.load_adapter(lang_adapter_dir)
    ...
    pretrained_task_name = model.load_adapter(f"{checkpoint}/wikiann-task-adapter")
    pretrained_task_modules = model.get_adapter(pretrained_task_name)
    task_cached_weights = {}
    for layer_i, v in pretrained_task_modules.items():
        for layer_name in v.keys():
            for name, param in pretrained_task_modules[layer_i][layer_name].named_parameters():
                task_cached_weights[f"{layer_i}-{layer_name}-{name}"] = param.data
    model.delete_adapter(pretrained_task_name)

    model.add_adapter("wikiann_union_adapter", config=config)
    model.set_active_adapters("wikiann_union_adapter")
    ...
    for layer_i, v in union_adapter_modules.items():
        for layer_name in v.keys():
            for name, param in union_adapter_modules[layer_i][layer_name].named_parameters():
                # ensure the layer name doens't exist simultaneously in 
                # task adapters and language adapters
                assert not (f"{layer_i}-{layer_name}-{name}" in cached_weights and f"{layer_i}-{layer_name}-{name}" in task_cached_weights)
                ...
                if f"{layer_i}-{layer_name}-{name}" in task_cached_weights:
                    # print(f"Replacing {layer_i}-{layer_name}-{name}")
                    param.data = task_cached_weights[f"{layer_i}-{layer_name}-{name}"]
                    param.requires_grad = False
...
for seed in range(args.seed_runs):
    ...
    checkpoints_dir = list(pathlib.Path(f"{args.en_adpt_dir}/seed{seed}").glob("checkpoint-*"))
    ...
    if "_pfeiffer_" in model_name:
        ...
        def model_init():
            ...
            model = load_config_adapters(model, config, f"{model_name}/oscar_ia3_{language}")
            model.eval()
            return model
        ...
        def model_init():
            ...
            model = load_config_adapters(model, config, f"{model_name}/oscar_lora_{language}")
            model.eval()
            return model
...
print("="*50)
print(f"Results WikiANN-{language}")
print("="*50)
...
with open(f"{model_name}/wikiann-{language}-results.txt", "w+") as wf:
    wf.write("="*50)
    wf.write('\n')
    wf.write(f"Results WikiANN-{language}")
    wf.write('\n')
    wf.write("="*50)
    wf.write('\n')
