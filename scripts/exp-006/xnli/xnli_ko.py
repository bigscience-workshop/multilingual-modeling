...
def en_tokenize_function(examples):
    if KLUE_TASKS[args.klue_task].klue_split == "ynat":
        return en_tokenizer(examples["title"], max_length=128, padding="max_length", truncation=True)
    elif KLUE_TASKS[args.klue_task].klue_split == "sts":
        return en_tokenizer(f'{examples["sentence1"]} {en_tokenizer.eos_token} {examples["sentence2"]}', max_length=128, padding="max_length", truncation=True)
    elif KLUE_TASKS[args.klue_task].klue_split == "nli":
...
if args.zero_shot:
    ...
    full_train_dataset = train_dataset.map(en_tokenize_function, batched=False).map(postprocessing)
    full_val_dataset = val_dataset.map(en_tokenize_function, batched=False).map(postprocessing)
else:
